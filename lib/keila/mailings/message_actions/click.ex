defmodule Keila.Mailings.MessageActions.Click do
  use Keila.Repo
  import Keila.PipeHelpers
  alias Keila.Mailings.Message

  @doc """
  Runs side-effects associated with a link in a message being clicked.
  ...
  defp maybe_update_message(message_id, min_delay) do
    query =
      from(r in Message,
        where: r.id == ^message_id and is_nil(r.clicked_at),
  ...
  defp log_event(%Message{id: message_id, contact_id: contact_id}) do
    Keila.Tracking.log_event("click", contact_id, message_id, %{})
  end
end
